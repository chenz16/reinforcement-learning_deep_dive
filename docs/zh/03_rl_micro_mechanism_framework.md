...
原始 Policy Gradient 的问题在 score 项的方差：

```text
原始 PG：  ∇J = E[∇log π(a|s) · G_t]
                                  ↑
                        Monte Carlo return，方差极大
                        一条轨迹好坏完全取决于采样运气
```
...
```text
Step 1: Actor-Critic 引入 V baseline
  ∇J = E[∇log π(a|s) · (Q(s,a) - V(s))]
  = E[∇log π(a|s) · A(s,a)]
  用 V(s) 减去 "state 本身的平均水平"
  → 只保留 "这个 action 相对平均的超额收益"
  → 方差大幅降低

Step 2: GAE 进一步优化 advantage 估计
  不显式估 Q，用 V + rollout reward 构造多步 advantage
  δ_t = r_t + γV(s_{t+1}) - V(s_t)          ← 单步 TD error
  A^GAE_t = Σ_l (γλ)^l δ_{t+l}               ← 多步加权平均

  λ=0: 只看一步 TD → 低方差、高偏差
  λ=1: 看所有步 TD → 高方差、低偏差
  λ=0.95（典型值）: 在中间取最佳折中

  这是让 policy gradient 从 "高方差采样估计"
  变成 "可用的稳定优化问题" 的关键。
```

**这是 PPO 稳定性的底层信号质量**——没有 GAE + V-critic，后面的 clip 和 entropy 再好也没用，因为 gradient 方向本身就是噪声。

#### 更新幅度控制：Clipped Ratio（护栏）

GAE 解决了"gradient 方向准不准"的问题。Clip 解决的是另一个问题："即使方向对了，一步走太远也会崩"。
...
如果 r 偏离 1 太多（新旧 policy 差太大）：
  → clip 限制 loss 的大小
  → 防止单次更新把 policy 推飞
```

Clip 是 TRPO 思想的简化版。TRPO 用 KL 约束（二阶优化，复杂），PPO 用 clip（一阶优化，简单）。效果接近，工程成本低很多。
...
│ 层级                 │ 作用                                  │
├──────────────────────┼───────────────────────────────────────┤
│ GAE + V-critic       │ 信号质量：让 gradient 方向准            │
│ (最本质)             │ 解决 PG 最根本的高方差问题               │
│                      │ 没有它，后面的一切都没用                 │
├──────────────────────┼───────────────────────────────────────┤
│ Clipped ratio        │ 步长控制：让每次更新不走过头              │
│ (护栏)               │ 即使方向对了，走太远也会崩               │
│                      │ TRPO 的工程简化版                      │
├──────────────────────┼───────────────────────────────────────┤
│ Near on-policy       │ 数据质量：数据来自最近 policy             │
│ (数据保障)            │ 不用 replay buffer，分布干净             │
│                      │ 避免 off-policy 的分布漂移问题           │
├──────────────────────┼───────────────────────────────────────┤
│ Entropy bonus        │ 探索保障：防止 policy 过早塌缩            │
│ (辅助)               │ 在 PPO 中通常是小系数，辅助作用           │
├──────────────────────┼───────────────────────────────────────┤
│ 多 epoch minibatch   │ 工程效率：一批数据用多次                  │
│ (工程)               │ 提高 near on-policy 数据的利用率          │
│                      │ 简单、好并行、好调参                     │
└──────────────────────┴───────────────────────────────────────┘
```
...
```text
GAE 不是 PPO 原创——GAE 是 Schulman 2016 的独立论文。
Clipped surrogate objective 才是 PPO (Schulman 2017) 的核心贡献。

PPO 的真正贡献是组装：
  把 GAE（稳定的 advantage 估计）
  + clipped ratio（简单的步长约束）
  + 多 epoch minibatch（工程友好）
  组合成了一个工业可用的算法。
...
```text
PPO 的 clip ratio 是工程上漂亮的护栏，
但真正让 policy gradient 从 "高方差采样估计" 变成可用优化问题的，
是 V-critic baseline 和 GAE advantage estimator。
...
## 14. RL 对 SL/SSL 的思想反哺：自举式学习

前面我们一直在用 SL 的视角来理解 RL。但反过来，RL 也对 SL 和自监督学习（SSL）做出了一个深层贡献：**自举式目标构造——训练目标不来自外部标签，而来自模型自身的估计。**

### 14.1 SL、RL、SSL 的标签来源对比

```text
SL：label 来自外部
  数据: (x, y)
  y 是人工标注的，模型训练过程不改变 y
  模型被动地拟合固定标签

RL：target 来自自己的估计（bootstrap）
  Q(s,a) ← r + γ max_a' Q(s',a')
  target 不是外部给的，而是模型用自己对未来的估计构造出来的
  模型自己造 label，然后再拟合这个 label

SSL：target 来自数据自身的结构
  Masked LM：遮住一个 token，预测被遮的 token（label = 原始 token）
  Contrastive：同一图像的两个 augmentation 应该表示接近（label = 自己）
  BYOL/DINO：student 追 teacher/momentum model（label = 自己的慢速版本）
```

### 14.2 RL 的 bootstrap 思想如何被 SSL 吸收
...
这个思想后来被 SSL 广泛吸收——只是自举的对象不同：

| 领域 | 自举对象 | target 来源 | 学到什么 |
|---|---|---|---|
| RL / TD | Q / V value | reward + next-state value estimate | 行为价值 |
| DQN | Q network | target network 生成的 TD target | action value |
| BYOL / DINO | student representation | teacher / momentum encoder | 语义表示 |
| Masked LM (BERT) | missing token | 原始数据自身 | 语言结构 |
| MAE | missing image patch | 原始像素 | 视觉结构 |
| Diffusion Model | clean data | 从 noisy data 预测 noise/clean | 数据分布 |

### 14.3 共同的底层哲学

```text
SL 的世界观：有上帝标签，模型的任务是拟合。
RL 的世界观：没有上帝标签，只有采样、反馈和自举。
SSL 的世界观：没有人工标签，只有数据自身结构和模型生成的目标。

RL 和 SSL 共享一个核心信念：
  学习系统不必完全依赖外部标签。
  它可以通过采样、扰动、预测和自举来制造自己的监督信号。
```

这也跟我们前面讲的 RL 演化弧完全吻合：

```text
从"上帝视角的精确优化"（DP，已知模型，已知 reward）
走向"没有上帝视角的统计学习"（采样、偏好、自举、迭代修正）

这条路不只是 RL 自己走过的，也是整个现代 ML 走过的：
  SL（需要标签）→ SSL（不需要标签，自构造目标）→ 生成模型（学整个分布）
```

### 14.4 一句话总结

```text
RL 对现代 ML 的深层贡献，不只是 reward 和 policy，
而是它很早就把学习从"固定标签监督"推进到了"自举式目标构造"。

TD learning 的 bootstrap 思想 → SSL 的 self-constructed target
这是同一个哲学在不同领域的实现。
```
